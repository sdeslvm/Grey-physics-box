...
protocol GreyPhysicsLoadStateRepresentable {
    var type: GreyPhysicsLoadState.StateType { get }
...
struct GreyPhysicsLoadState: Equatable, GreyPhysicsLoadStateRepresentable {
    ...
    static func idle() -> GreyPhysicsLoadState {
        GreyPhysicsLoadState(type: .idle, percent: nil, error: nil)
    }

    /// Создание состояния прогресса
    static func progress(_ percent: Double) -> GreyPhysicsLoadState {
        GreyPhysicsLoadState(type: .progress, percent: percent, error: nil)
    }

    /// Создание состояния успеха
    static func success() -> GreyPhysicsLoadState {
        GreyPhysicsLoadState(type: .success, percent: nil, error: nil)
    }

    /// Создание состояния ошибки
    static func error(_ err: String) -> GreyPhysicsLoadState {
        GreyPhysicsLoadState(type: .error, percent: nil, error: err)
    }

    /// Создание состояния отсутствия подключения
    static func offline() -> GreyPhysicsLoadState {
        GreyPhysicsLoadState(type: .offline, percent: nil, error: nil)
    }

    // MARK: - Методы сравнения

    /// Пользовательская реализация сравнения
    func isEqual(to other: GreyPhysicsLoadState) -> Bool {
    ...
    static func == (lhs: GreyPhysicsLoadState, rhs: GreyPhysicsLoadState) -> Bool {
        lhs.isEqual(to: rhs)
    }
}
...
extension GreyPhysicsLoadState {
...
extension GreyPhysicsLoadState: CustomStringConvertible {
