...
struct GreyPhysicsWebViewBox: UIViewControllerRepresentable {
    // MARK: - Свойства

    @ObservedObject var loader: GreyPhysicsWebLoader
    var defaultOrientations: UIInterfaceOrientationMask = .portrait

    // MARK: - Координатор

    func makeCoordinator() -> GreyPhysicsWebCoordinator {
        GreyPhysicsWebCoordinator { [weak loader] status in
    ...
    func makeUIViewController(context: Context) -> GreyPhysicsBaseWebViewController {
        let configuration = createWebViewConfiguration()
        return GreyPhysicsBaseWebViewController(
    ...
    func updateUIViewController(_ uiViewController: GreyPhysicsBaseWebViewController, context: Context) {
